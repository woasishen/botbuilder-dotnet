...
namespace Microsoft.Bot.Connector.Client.Authentication
{
    internal class ParameterizedBotFrameworkAuthentication : BotFrameworkAuthentication
    {
        ...
        public override async Task<ClaimsIdentity> AuthenticateChannelRequestAsync(string authHeader, CancellationToken cancellationToken)
        {
            ...
            {
                var isAuthDisabled = await _credential.IsAuthenticationDisabledAsync(cancellationToken).ConfigureAwait(false);
                if (!isAuthDisabled)
                {
        ...
        public override async Task<AuthenticateRequestResult> AuthenticateRequestAsync(Activity activity, string authHeader, CancellationToken cancellationToken)
        {
            ...
            var callerId = await GenerateCallerIdAsync(_credential, claimsIdentity, _callerId, cancellationToken).ConfigureAwait(false);

            var connectorFactory = new ConnectorFactoryImpl(_credential);
        ...
        {
            if (string.IsNullOrWhiteSpace(channelIdHeader) && !await _credential.IsAuthenticationDisabledAsync(cancellationToken).ConfigureAwait(false))
            {
                throw new UnauthorizedAccessException();
            }
        ...
        {
            return new ConnectorFactoryImpl(_credential);
        }
        ...
        public override Task<UserTokenClient> CreateUserTokenClientAsync(ClaimsIdentity claimsIdentity, CancellationToken cancellationToken)
        {
            ...
            var client = new ConnectorFactoryImpl(_credential).CreateUserTokenClient(new Uri(_oAuthUrl), appId);
        ...
        /// </summary>
        /// <param name="credential">A <see cref="BotFrameworkCredential"/> to use.</param>
        ...
        private static async Task<string> GenerateCallerIdAsync(BotFrameworkCredential credential, ClaimsIdentity claimsIdentity, string callerId, CancellationToken cancellationToken)
        {
            // Is the bot accepting all incoming messages?
            if (await credential.IsAuthenticationDisabledAsync(cancellationToken).ConfigureAwait(false))
            {
            ...
            return claimsIdentity.Claims.IsSkillClaim()
                ? $"{CallerIdConstants.BotToBotPrefix}{claimsIdentity.Claims.GetAppIdFromClaims()}"
                : callerId;
        }
        ...
        private async Task<ClaimsIdentity> JwtTokenValidation_AuthenticateRequestAsync(Activity activity, string authHeader, CancellationToken cancellationToken)
        {
            if (string.IsNullOrWhiteSpace(authHeader))
            {
                ...
                // TODO: Add skill to RoleTypes
                //if (activity.ChannelId == Channels.Emulator && activity.Recipient?.Role == RoleTypes.Skill)
                if (activity.ChannelId == BotFrameworkChannels.Emulator && activity.Recipient?.Role.ToString() == "skill")
                {
        ...
        private async Task ValidateSkillIdentityAsync(ClaimsIdentity identity, CancellationToken cancellationToken)
        {
            ...
            if (!await _credential.IsValidAppIdAsync(audienceClaim, cancellationToken).ConfigureAwait(false))
            {
        ...
        private async Task<ClaimsIdentity> AuthenticateEmulatorTokenAsync(string authHeader, string channelId, CancellationToken cancellationToken)
        {
            ...
            if (!await _credential.IsValidAppIdAsync(appId, cancellationToken).ConfigureAwait(false))
            {
        ...
        private async Task ValidateChannelIdentityAsync(ClaimsIdentity identity, string serviceUrl, CancellationToken cancellationToken)
        {
            ...
            if (!await _credential.IsValidAppIdAsync(appIdFromClaim, cancellationToken).ConfigureAwait(false))
            {
                // The AppId is not valid. Not Authorized.
